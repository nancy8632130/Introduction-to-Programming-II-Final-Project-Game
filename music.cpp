#include "music.h"
#include <stdlib.h>
Music::Music(char *path, int volume, int isLoop)
{
    media=new QMediaPlayer;
    media->setMedia(QUrl::fromLocalFile(path));
    media->setVolume(volume);
    isConnectBefore=0;
    if(isLoop){
        setPlayInLoop();
        isConnectBefore=1;
    }


}
Music::~Music()
{

}
void Music::setPlayOneTime(){
    disconnect( media, SIGNAL(stateChanged(QMediaPlayer::State)), media, SLOT(play()));
}
void Music::setPlayInLoop(){
    connect( media, SIGNAL(stateChanged(QMediaPlayer::State)), media, SLOT(play()));
}
void Music::stop(){

    isConnectBefore = disconnect( media, SIGNAL(stateChanged(QMediaPlayer::State)), media, SLOT(play()));
    media->stop();
}
void Music::pause(){
    isConnectBefore = disconnect( media, SIGNAL(stateChanged(QMediaPlayer::State)), media, SLOT(play()));
    media->pause();
}
void Music::play(){
    if(isConnectBefore)
        setPlayInLoop();
    else
        setPlayOneTime();
    media->play();
}
void Music::setVolume(int V){
    media->setVolume(V);
}
void Music::setMusicPath(char *p){
    media->setMedia(QUrl::fromLocalFile(p));
}
void Music::mute(bool m){
    media->setMuted(m);
}
void Music::louderOrlower(bool L){
    if(L==true)
        media->setVolume(media->volume()+10);
    else
        media->setVolume(media->volume()-10);

}
